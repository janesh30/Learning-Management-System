    ...
    public static Parent build(Stage stage) {
        ...
        subBtn.setOnAction(e -> {
            ...
            if(sel != null && !sel.deadline.isBefore(LocalDateTime.now())) {
                ...
                if(f != null) DataService.uploadFile(f, "INSERT INTO LMS_SUBMISSIONS (assignment_id, student_name, filename, file_data) VALUES(?,?,?,?)", DataService.currentUsername, sel.id);
            } else DataService.showAlert("Select an OPEN assignment first.");
        });
